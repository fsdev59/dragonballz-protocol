...
import { getProposal, getQuorumVotes, getProposalStatus, castVote } from '../../dragonballzUtils'
...
const ProposalPage: React.FC = () => {
  ...
  const handleVoteForClick = useCallback(() => {
    castVote(dragonballz, proposal.id, true, account)
  }, [account, dragonballz, proposal.id])

  const handleVoteAgainstClick = useCallback(() => {
    castVote(dragonballz, proposal.id, false, account)
  }, [account, dragonballz, proposal.id])

  const fetchProposal = useCallback(async () => {
    const proposal = await getProposal(dragonballz, proposalId)
    setProposal(proposal)
  }, [dragonballz, proposalId])

  const fetchVotes = useCallback(async () => {
    const proposalStatus: ProposalStatus = await getProposalStatus(dragonballz, proposalId)
    ...
    const quorumCount = await getQuorumVotes(dragonballz)
    setVotes({
  ...
  }, [fetchProposal, fetchVotes, dragonballz])
